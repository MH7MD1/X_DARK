...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                try:
                    ...
                    await lel.edit(
                        "<b><i>من فضلك اعطني صلاحية الادمن🙋‍♀️🥀</i></b>")
                    return

                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message.chat.id, "انضم الحساب المساعد🧚‍♀️🥀")

                except UserAlreadyParticipant:
                    pass
                except Exception:
                    await lel.edit(
                        f"<b><i>الحساب المساعد محظور من دردشتك🤷‍♀️</i></b>")
    
    try:
        await USER.get_chat(chid)
    except:
        await lel.edit(
            f"<i><b>خطأ {user.first_name}, من فضل اضف الحساب المساعد قبل التشغيل💁‍♂️</b></i>")
        return
    ...
    if audio:
            ...
            [
                [
                    InlineKeyboardButton(
                            text="تيم اكس🥀",
                            url=f"https://t.me/X_DARK1"),
                    InlineKeyboardButton(
                            text="التحديثات👾",
                            url=f"https://t.me/CHVOTLX"),
               ],
               [
                   InlineKeyboardButton(text="اغلاق القائمه", callback_data="close_"),                   
               ],
            ]
        )
        ...
        if (dur / 360) > DURATION_LIMIT:
            await lel.edit(
                f"**اعطني اسم اغنيه اكبر من {DURATION_LIMIT} **"
            )
            return
        requested_by = message.from_user.first_name
        await generate_cover(requested_by, title, views, duration, thumbnail)
        ...
        if (dur / 60) > DURATION_LIMIT:
            await lel.edit(
                f"**اعطني اغنيه اكبر من {DURATION_LIMIT} **"
            )
            return
        requested_by = message.from_user.first_name
        await generate_cover(requested_by, title, views, duration, thumbnail)
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            caption="****تم اضافة الاغنيه الي قائمة الانتظار🥀💖 \n\nالطلب بواسطة: {} \nفي الدور :-** {}**".format(usrid, position),
